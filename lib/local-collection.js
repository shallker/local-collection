...
module.exports = function LocalCollection(name) {
  ...
  */
  ...
  /*
    Search all the records which match selector
    @arguments [Object selector]
  */
  collection.all = function (selector) {
    if (selector) {
      var result = [];
  ...
  /*
    Search one record which match selector first
    @arguments Object selector
  */
  collection.one = function (selector) {
    for (var i in records) {
      if (match(records[i], selector)) return records[i];
    }
